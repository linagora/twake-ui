...
// twake-mui declares its variables under a plain [data-theme="…"] attribute
// selector, so an offscreen node carrying that attribute resolves the other
// mode without touching the page.
...
// Everything here must be read after mount: twake-mui injects its stylesheet
// during the first commit, so nothing is declared or resolvable before then.
interface Resolved {
  palette: string[]
  shadows: string[]
...
export const CssVariables: FC = () => {
  ...
  const visible = palette.filter(
    name =>
      // Channel and opacity variants are plumbing, not colors to reach for.
      !/(channel|opacity)$/i.test(name) &&
      // MUI derives per-component colors under a capitalised prefix
      // (--twake-palette-Alert-errorColor). Internal, not design tokens.
  ...
  return (
    <Chapter id="variables" title="Theme variables">
      <Prose>
        These come from twake-mui, not from this package — they exist in the
        page only while a provider is mounted, and any of them can be used in
        your own CSS. Colors are resolved in both modes below.
      </Prose>
